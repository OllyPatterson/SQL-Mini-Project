...
--Using Case statements to identify what number corresponds to the correct description
     END AS DESCRIPTION
--Above I have assigned the surrogate keys of the original dataset with the descriptions from the HES data dictionary
--This way, there is a readable description for each attribute that does not include a surrogate reference
INTO DimAdminCat
--This has then been selected into a stand-alone table renamed 'DimAdminCat' for the original attribute 'AdminCat' 

FROM GWS
--GWS is the original dataset I imported from Excel
...
--Below is the code to Join the above Reference tables together into one readable version of the data
select 
gws.[ID no#]
,gws.hesid as HesID
...
--Casting the date attributes as dates (which were previously varchar) so I can use the DATEDIFF function in the following code:
...
--using isnull to further correct the presence of NULLS by replacing them with a consistent use of 'Not Known' - as seen in the HES syntax of the original data
,DimAdmisorc.description as Admisorc
,Dimelecdur.description as Elecdur
,Dimclasspat.description as Classpat
/*
into GWSFINAL
Seen directly above, I have commented out the final section which selects the big join into a standalone table of its own named 'GWSFINAL'
